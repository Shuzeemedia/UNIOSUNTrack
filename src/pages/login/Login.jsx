...
// import { loginWebAuthn } from "../../utils/webauthn";
...
const Login = () => {
    ...
    try {
      ...
      if (user.role === "student") {
        if (!user.faceDescriptor || user.faceDescriptor.length !== 128) {
          toast.info("Please enroll your face before using attendance.");
  ...
  //           if (headTurned) {
  //             recognized = true;
  //             setStatusMessage("Head movement detected. Verifying face...");
        ...
        <form onSubmit={handleSubmit}>
          ...
          {/* <button
            ...
            onClick={async () => {
              try {
                setLoading(true);
                const data = await loginWebAuthn(); 
                ...
                if (data.user.role === "student") {
                  if (!data.user.faceDescriptor || data.user.faceDescriptor.length !== 128) {
                    toast.info("Please enroll your face before using attendance.");
                    navigate("/enroll-face");
                  } else {
                    navigate("/dashboard/student");
                ...
                toast.success("Logged in with device!");
              } catch (err) {
                console.error(err);
